...
specialized solutions are needed, but there is a range of medium-sized problems
that can be successfully tackled on a computer cluster with a few hundred nodes
or less (so that a computation that would take a CPU-year can produce results
in less than a day).
...
This makes jug usable in batch-based computer cluster environments, which are
quite common in research institutions.
...
\url{http://neuralensemble.org/trac/sumatra}.}). Additionally, minute
...
through February~2012 and has been downloaded over 10,000~times (some of these
